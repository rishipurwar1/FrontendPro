...
const ChallengeCard = ({ challenge, challengelist, btnTitle }) => {
    ...
    return (
        <div className="bg-gray-800 shadow-2xl overflow-hidden rounded-md max-w-sm">
            <Image className="max-h-50" cloudName="di5hmgowi" alt={`${challenge.title} Challenge`} loading="lazy" public-id={challenge.images.cover}>
            ...
            <Link to={link} className="block rounded-t-none mt-4 w-full bg-gradient-to-br from-purple-500 to-indigo-500 text-purple-200 font-semibold font-heading text-lg text-center px-5 py-5" title={`This is a link to ${challenge.title} challenge detail page`}>{btnTitle}</Link>
        </div>
    )
}
